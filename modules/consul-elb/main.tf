...
# ---------------------------------------------------------------------------------------------------------------------
# CREATE THE CONSUL ELB
# ---------------------------------------------------------------------------------------------------------------------

resource "aws_elb" "consul-servers" {
  name = var.name
  ...
  security_groups = [aws_security_group.consul-servers.id]
...
# ---------------------------------------------------------------------------------------------------------------------
# ATTACH THE ELB TO THE CONSUL ASG
# ---------------------------------------------------------------------------------------------------------------------

resource "aws_autoscaling_attachment" "consul-servers" {
  autoscaling_group_name = var.consul_asg_name
  elb                    = aws_elb.consul-servers.id
}
...
resource "aws_security_group_rule" "allow_inbound_consul_api" {
  type        = "ingress"
  ...
  security_group_id = aws_security_group.consul-servers.id
}

resource "aws_security_group_rule" "allow_all_consul_outbound" {
  type        = "egress"
  from_port   = 0
  to_port     = 0
  protocol    = "-1"
...
resource "aws_route53_record" "consul_elb" {
  ...
  name    = "${var.consul_domain_name}"
  type    = "A"

  alias {
    name    = aws_elb.consul-servers.dns_name
    zone_id = aws_elb.consul-servers.zone_id
